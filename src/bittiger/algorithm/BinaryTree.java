 ...
 * getLastCommonParent, getLastCommonParentRec, getLastCommonParentRec2
 ...
 */
	public static TreeNode rebuildBinaryTreeRec(List<Integer> preOrder, List<Integer> inOrder){
        ...
        // 因为知道root节点了，所以根据root节点位置，把preorder，inorder分别划分为 
        //root左侧 和 右侧 的两个子区间
	...
	while( !queue.isEmpty() ){
		...
		if(mustHaveNoChild){	  
			...
			if(cur.left!=null && cur.right!=null){		
				queue.add(cur.left);
				queue.add(cur.right);
				// 如果左子树非空但右子树为空，说明已经出现空节点，之后必须都为空子树
			}else if(cur.left!=null && cur.right==null){	
				mustHaveNoChild = true;
				queue.add(cur.left);
				// 如果左子树为空但右子树非空，说明这棵树已经不是完全二叉完全树！
			}else if(cur.left==null && cur.right!=null){	
				result = false;
				break;
